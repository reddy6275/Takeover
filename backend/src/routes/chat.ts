    ...
    if (sender === 'customer') {
      ...
      const { data: aiMessage } = await supabase
        ...
        .insert({
          ...
          citations: aiResult.citations.length > 0 ? aiResult.citations.map((c: any) => ({
            chunk_id: c.id,
...
function analyzeSentimentLexicalLocal(text: string): 'happy' | 'neutral' | 'angry' | 'frustrated' | 'urgent' {
  const lower = text.toLowerCase();
  if (lower.includes('urgent') || lower.includes('asap') || lower.includes('emergency')) return 'urgent';
  if (lower.includes('hate') || lower.includes('angry') || lower.includes('worst')) return 'angry';
  if (lower.includes('annoyed') || lower.includes('frustrated') || lower.includes('useless')) return 'frustrated';
  if (lower.includes('thank') || lower.includes('love') || lower.includes('great')) return 'happy';
  return 'neutral';
}
...
router.post('/stream', async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const contextBlock = retrievedChunks.length > 0
      ? retrievedChunks.map((chunk: any, index: number) => `[Source ${index} - Document: ${chunk.filename}]\n${chunk.content}`).join('\n\n---\n\n')
...
${content}`;
    ...
    const citationsMeta = retrievedChunks.map((c: any, index: number) => ({
      index,
      chunk_id: c.id,
      document_id: c.document_id,
      filename: c.filename
    ...
    await supabase.from('messages').insert({
      ...
      sender: 'ai',
      content: fullAnswer,
      citations: retrievedChunks.length > 0 ? retrievedChunks.map((c: any) => ({
        chunk_id: c.id,
        document_id: c.document_id,
        filename: c.filename
      })) : null
 ...
 */
router.post('/messages/:id/feedback', async (req: Request, res: Response): Promise<void> => {
  const { id } = req.params;
