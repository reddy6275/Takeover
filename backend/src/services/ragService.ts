...
export const INDUSTRY_PROMPT_TEMPLATES: Record<string, string> = {
...
export async function parseDocument(buffer: Buffer, fileTypeOrName: string): Promise<string> {
...
export async function generateGroundedAnswer(
  ...
  companyName: string,
  aiTone: string
): Promise<{ answer: string; confidence: number; shouldCreateTicket: boolean; citations: any[] }> {
  const { data: company } = await supabase
    .from('companies')
  ...
  if (isGeminiConfigured()) {
    try {
      ...
      if (
        answer.includes("don't have that information") || 
